...
def build_episode_fragment_prompt(template: str, episode: dict, fragment: dict,
    ...
    payload = {
        "任务说明": {
            "唯一任务": "只为【当前片段】生成 Seedance JSON，不要生成其他片段。",
            "核心依据": "【当前片段】是本次需要转成视频提示词的唯一正文，人物、动作、台词、场景和情节必须以它为准。",
            "上下文用途": "【当前集完整剧本原文（上下文）】是当前集从开头到结尾的完整原文，只用于理解当前片段的前因后果、人物关系、情绪和剧情位置。不得把原文中其他片段的事件写进当前片段。",
            "连续性用途": "【上一片段生成结果】只用于承接人物状态、空间位置、光线和镜头连续性，不得续写上一片段内容。为空时忽略。",
            "设定用途": "【基本设定】用于统一年代、视觉风格、拍摄方式、色调和限制条件。",
            "冲突优先级": "如信息冲突，按【当前片段】>【当前集完整剧本原文（上下文）】>【上一片段生成结果】的优先级判断；【基本设定】中的风格和限制始终遵守。",
        },
        "输入内容": {
            "基本设定": ai_settings,
            "集数": episode_no,
            "剧集标题": episode.get("title", f"第{episode_no}集"),
            "当前集完整剧本原文（上下文）": episode.get("source", ""),
        ...
        "输出要求": {
            "格式": "严格 JSON 对象，不要代码块、解释或提问",
            "范围": "仅输出当前片段的结果；可含 meta 和 fragments，但 fragments 只能对应当前片段",
            "禁止事项": "不得概括整集，不得生成其他片段，不得把上下文中的其他事件、动作或台词混入当前片段",
